...
/*
produce is used to produce the data to a topic
usage is defined as:

std::string brokers = "localhost:9092";
std::topic = "anomaly-topic";
std::message = "this is an anomaly";

if(produce(brokers, topic, message)) {
...
bool produce(std::string brokers, std::string topic, std::string msg_to_send) {
...
retry:
    RdKafka::ErrorCode err =
        producer->produce(
            ...
            /* Value */
            const_cast<char *>(msg_to_send.c_str()), msg_to_send.size(),
            /* Key */
            NULL, 0,
    ...
    if (err != RdKafka::ERR_NO_ERROR) {
        ...
        std::cerr << "% Enqueued message (" << msg_to_send.size() << " bytes) " <<
